...
if (root) {
    ...
    let telemetry = {
        voltage: metricsSeed?.voltage ?? 0,
        current: metricsSeed?.current ?? 0,
        battery: metricsSeed?.battery ?? 90,
    };
    ...
    const applyStatusStyles = () => {
        ...
        selectors.statusBadge.classList.add(...(on ? statusClasses.on : statusClasses.off));
        selectors.toggleButton.classList.remove(...toggleClasses.on, ...toggleClasses.off);
        selectors.toggleButton.classList.add(...(on ? toggleClasses.on : toggleClasses.off));

        if (selectors.statusText) {
            selectors.statusText.textContent = state?.statusLabel ?? (on ? 'Bomba activa' : 'Bomba apagada');
        }

        if (selectors.toggleLabel) {
            selectors.toggleLabel.textContent = on ? 'Apagar bomba' : 'Encender bomba';
        }

        if (selectors.statusDescription) {
            selectors.statusDescription.textContent = on ? DESCRIPTIONS.on : DESCRIPTIONS.off;
        }

        if (selectors.runtimeState) {
            selectors.runtimeState.textContent = on ? 'En ejecución' : 'En espera';
        }
    };
    ...
    const updateReadouts = () => {
        if (selectors.metricVoltage) {
            selectors.metricVoltage.textContent = telemetry.voltage.toFixed(1);
        }
        if (selectors.metricCurrent) {
            selectors.metricCurrent.textContent = telemetry.current.toFixed(2);
        }
        if (selectors.metricBattery) {
            selectors.metricBattery.textContent = Math.round(telemetry.battery).toString();
        }
        if (selectors.batteryBar) {
            selectors.batteryBar.style.width = `${clamp(telemetry.battery, 0, 100)}%`;
        }
    };
    ...
    const applyState = (nextState, nextSeed = null) => {
        state = {
            ...state,
            ...nextState,
        };
    ...
    const togglePump = async () => {
        ...
        try {
            ...
            applyState(payload.state, payload.metricsSeed ?? null);

            const actionTitle = state.isOn ? 'Bomba encendida' : 'Bomba apagada';
            const actionDetails = state.isOn
                ? 'El simulador comienza a generar consumo eléctrico y tiempo operativo.'
                : 'Se detiene la simulación y se conserva el tiempo acumulado.';
            logActivity(actionTitle, actionDetails, state.isOn ? 'positive' : 'warning');
        } catch (error) {
            console.error('No se pudo alternar la bomba', error);
            logActivity('Error de simulación', 'No fue posible enviar el comando. Revisa la conexión.', 'warning');
        } finally {
